...
namespace GLFrameworkEngine
{
    ...
    public class ObjLoader
    {
        ...
        public void LoadObj(Stream objStream) {
            ParseObj(objStream);
        }
                        ...
                        case "f":
                            ...
                            for (int i = 0; i < 3; i++)
                            {
                                ...
                                if (vertexArgs.Length > 1 && vertexArgs[1] != String.Empty) {
                                    vertex.TexCoord = TexCoords[Int32.Parse(vertexArgs[1]) - 1];
                                }
                                if (vertexArgs.Length > 2 && vertexArgs[2] != String.Empty) {
                                    vertex.Normal = Normals[Int32.Parse(vertexArgs[2]) - 1];
                                }

                                if (!currentPolygon.Vertices.Contains(vertex))
                                    currentPolygon.Vertices.Add(vertex);

                                int index = currentPolygon.Vertices.IndexOf(vertex);
                                currentPolygon.Indices.Add(index);
