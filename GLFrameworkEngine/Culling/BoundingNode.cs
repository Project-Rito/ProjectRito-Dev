...
namespace GLFrameworkEngine
{
    ...
    public class BoundingNode
    {
        ...
        /// <summary>
        /// Apply the scale during a bounding transform.
        /// </summary>
        ...
        public void Include(BoundingNode node)
        {
            Radius = MathF.Max(Radius, node.Radius);
            Box.Include(node.Box);
        }

        public static BoundingNode FromSphere(Vector3 position, float radius)
        {
            var bounding = new BoundingNode();
            bounding.Center = position;
            bounding.Radius = radius;
            return bounding;
        }

        public static BoundingNode FromBox(Vector3 min, Vector3 max) {
            return new BoundingNode(min, max);
        }
