...
namespace AGraphicsLibrary
{
    ...
    public struct ProbeGrid
    {
        ...
        {
            x = (uint)Math.Floor((worldPosition.X - Min.X) / Step.X);
            y = (uint)Math.Floor((worldPosition.Y - Min.Y) / Step.Y);
            z = (uint)Math.Floor((worldPosition.Z - Min.Z) / Step.Z);
        }
