...
namespace AGraphicsLibrary
{
    public class LightingEngine
    {
        ...
        public void SaveArchive(Dictionary<string, byte[]> files)
        {
            ...
            {
                if (Resources.ColorCorrectionFiles.ContainsKey(file.Key))
                    files[file.Key] = Resources.ColorCorrectionFiles[file.Key].Save(isVersion2);
                if (Resources.EnvFiles.ContainsKey(file.Key))
                    files[file.Key] = Resources.EnvFiles[file.Key].Save(isVersion2);
            }
        }
        ...
        public ProbeMapManager.ProbeOutput UpdateProbeCubemap(GLContext control, GLTextureCube probeMap, OpenTK.Vector3 position)
        {
            ...
            return ProbeMapManager.Generate(control, (GLTextureCube)lmap.Lightmaps[lightMapName], probeMap.ID, position);
        }
    }
}
