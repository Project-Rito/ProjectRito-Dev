...
namespace Toolbox.Core.Collada
{
    public class ColladaReader
    {
        ...
        static void TransformMeshInstanced(STGenericModel model, Node parent, bool applyMatrices)
        {
            if (NodeInstanceTransform.Any(x => x.Value == $"#{parent.ID}"))
            {
                foreach (var node in NodeInstanceTransform.Where(x => x.Value == $"#{parent.ID}"))
                {
                    ...
                    {
                        model.Meshes[index].Transform.Position = parent.Transform.ExtractTranslation();
                        model.Meshes[index].Transform.Scale = parent.Transform.ExtractScale();
                        model.Meshes[index].Transform.Rotation = parent.Transform.ExtractRotation();
                        model.Meshes[index].Transform.Update();
                    }
                }
            }

            foreach (var child in parent.Children)
                TransformMeshInstanced(model, child, applyMatrices);
        }
            ...
            STBone boneParent, ref Matrix4 parentTransform)
        {
            ...
            if (daeNode.sid != null && !BoneNameIds.ContainsKey(daeNode.sid))
            ...
            geometry geom, library_materials materials, controller controller = null)
        {
            ...
            for (int v = 0; v < mesh.Vertices.Count; v++)
            {
                ...
                if (scene.Settings.ApplyMatrices)
                {
                ...
                {
                    mesh.Transform.Position = node.Transform.ExtractTranslation();
                    mesh.Transform.Scale = node.Transform.ExtractScale();
                    mesh.Transform.Rotation = node.Transform.ExtractRotation();
        ...
        static Matrix4 CreateBindMatrix(string text)
        {
            if (text == null || text == "")
